#!/usr/bin/env python3
"""
Script to extract and download all embedded/ui/ assets from the game engine code.
"""
...
def find_embedded_ui_paths():
    """Extract all embedded/ui paths from JavaScript files."""
    pattern = re.compile(r'["\'](embedded/[^"\']+)["\']')
    paths = set()
...
def download_asset(asset_path):
    ...
    if output_path.exists() and output_path.stat().st_size > 0:
        return f"SKIP: {asset_path} (already exists)"
...
def main():
    print("Finding embedded/ui asset paths in JavaScript files...")
    paths = find_embedded_ui_paths()
