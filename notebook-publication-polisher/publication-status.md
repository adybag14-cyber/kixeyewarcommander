...
- Publication-readiness estimate: strong repaired executed notebook artifact, but still awaiting one clean rerun before full publication-ready sign-off

## Notebook state

- The polished notebook copy now opens with an exact quantitative title card and abstract: `111 of 225` retained traces, solvent order `Acetonitrile > Acetone > THF > Cyclohexane > Toluene`, a `46.5-fold` fastest-to-slowest span, and the bootstrap-rank stability result.
- The results discussion, post-lab answers, conclusion, and their underlying source cells now agree that acetonitrile is the fastest accepted solvent, acetone is second, and the literature comparison is partial because published comparator values are only available for acetone, THF, and cyclohexane in the saved package.
