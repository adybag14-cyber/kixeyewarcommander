...
- Patched the live notebook source so the title-card scope, abstract wording, configuration section, automated-check appendix note and reproducibility appendix now state plainly that the visible five-solvent results are archived executed outputs while the currently attached local package only exposes `testing-main/Data/Acetone`.
