...
- Confidence note: the polished notebook itself now contains the key fixes that had previously only been described in tracking notes. The saved artifact opens with captioned overflow-safe report tables instead of raw dataframe HTML, the weaker 2017 solvent-effects citation has been replaced with the 1987 *Chemical Physics Letters* primary paper, the final workflow GIF panel is collapsed by default, and the reduced-bundle provenance is stated explicitly in the notebook narrative and appendices.
...
- Replaced every remaining raw dataframe-style saved output with captioned, overflow-safe report tables for the environment audit, solvent summary, QC audit, bootstrap ranking, benchmark tables, validation summary, sensitivity table, and automated consistency checks.
...
- Corrected the remaining solvent-order interpretation drift in both the results discussion and post-lab answers so the notebook source and rendered prose now agree that acetonitrile is fastest and acetone is a close second.
