...
- Corrected the title page, reproducibility appendix and consistency-check appendix so the notebook no longer overstates the completeness of the attached local raw-data package.
...
- Added lazy-loading image attributes to the saved figure and GIF outputs and closed the laboratory workflow GIF panel by default so the notebook opens in a calmer reading state.
