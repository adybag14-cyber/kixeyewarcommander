...
- Overall state: the attached notebook package is now much closer to publication-ready because the strongest remaining narrative and presentation defects in the real saved file were corrected in a fresh polished copy that now exists in the workspace.
...
- Corrected the remaining research-quality interpretation drift in the results and post-lab narrative so acetonitrile is consistently treated as the fastest solvent, acetone as the close second, and the 46.5-fold span is stated explicitly where it matters.
