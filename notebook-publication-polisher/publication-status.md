...
- Confidence note: the polished notebook copy was rebuilt directly from the attached source, validated as readable JSON, checked for the corrected solvent-order prose, checked for the stronger reference list entry, checked for wrapped wide tables, checked for lazy-loaded media and a closed second GIF panel, and visually audited across all eight figures plus both GIF first frames.
...
- Corrected a second literature-comparison wording error in the discussion that had inaccurately described the acetone result relative to the published JCE benchmark.
