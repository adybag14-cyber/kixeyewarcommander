...
- Direct notebook-to-HTML export verification is still pending because this workspace does not currently provide `jupyter`/`nbconvert`, so the current render check is based on saved-output structure, embedded-media integrity, and extracted visual inspection rather than a fresh exported page.
