...
- Confidence note: the live attached notebook was re-audited directly in this run and brought back into line with the saved execution evidence. The opening scope note, configuration guidance, analysis-environment note and reproducibility appendix now state clearly that the notebook is being reviewed from archived five-solvent outputs while the currently attached local package only contains `testing-main/Data/Acetone`. The literature anchor is now a stronger primary ACS paper on solvent-controlled thermal isomerisation in a closely related aminoazobenzene system, the prose no longer contradicts the executed rate ordering, both inline GIF panels open in a cleaner closed state, and a refreshed embedded-media audit decoded all 10 saved visual assets successfully: 8 PNG figures and 2 inline GIFs.
