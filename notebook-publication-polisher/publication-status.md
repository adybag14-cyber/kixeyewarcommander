...
- Confidence note: the live attached notebook was re-audited directly in this run and brought back into line with the stronger publication state. The opening scope note, configuration guidance, live environment banner and reproducibility appendix now state clearly that the notebook is being reviewed from archived five-solvent outputs while the currently attached local package only contains `testing-main/Data/Acetone`. The stronger 1987 primary solvent-effects paper remains the literature anchor, the second inline GIF panel now opens in a cleaner closed state, and a refreshed embedded-media audit again decoded all 10 saved visual assets successfully: 8 PNG figures and 2 inline GIFs.
...
- Patched the real notebook source so the title-card scope, configuration section, live analysis-environment banner, reproducibility appendix and automated-check wording now state plainly that the displayed tables, figures and GIFs are archived outputs from a complete five-solvent run, while the currently attached review package only exposes `testing-main/Data/Acetone` locally.
