...
- Confidence note: this run rechecked the attachment itself, repaired the notebook source in place, rebuilt the polished output copy, restyled the saved numerical tables as captioned publication outputs, verified that both inline GIF panels are closed by default, and confirmed from the attached directory tree that only the acetone raw-data folder is present locally.
...
- Tightened the raw-data discovery error so an incomplete future rerun fails clearly and explains that the missing solvent directories must be restored.
