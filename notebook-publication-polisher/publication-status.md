...
- Recreated the missing polished deliverable at `/workspace/output/P201_201698955_publication_ready_polished.ipynb` from the attached notebook rather than relying on a previously generated workspace copy.
- Added explicit review-bundle and saved-execution provenance notes to the title card, configuration section, reproducibility appendix and consistency-check appendix so the saved five-solvent outputs are not misread as a fresh local rerun.
...
- Updated the notebook source so future reruns use the same publication-style HTML table wrapper and table captions instead of notebook-default dataframe rendering for the main report tables.
