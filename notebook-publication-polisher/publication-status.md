...
- Strengthened the conclusion so it closes with the retained-trace count, accepted solvent sequence, and chemical interpretation rather than a generic statement.
- Updated the appendix on reproducibility and portability so it states the real current blocker precisely: the saved executed notebook reflects the full five-solvent analysis, but the attached package available in this workspace contains only `Data/Acetone/`.
...
- Verified that both inline GIF panels remain collapsed by default; no default-open oversized media panels remain.
