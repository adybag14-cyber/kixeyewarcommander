...
- The saved notebook now states clearly that the visible figures, tables and GIFs are archived five-solvent outputs while the attached local bundle used for review only contains `Data/Acetone`, and the surrounding reproducibility and automated-check wording was brought into line with that same provenance language.
