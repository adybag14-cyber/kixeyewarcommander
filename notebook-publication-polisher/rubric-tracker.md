...
- Data analysis + plotting: very strong. The stored notebook contains all expected figures, summary tables, validation outputs, and embedded animations; `8` PNGs and `2` GIFs decoded cleanly in this pass; both expandable animation panels now start closed; the saved notebook-level styling reduces dataframe clipping risk; and the source-side table helper now keeps that protection on rerun.
