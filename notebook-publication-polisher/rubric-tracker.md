...
- Data analysis + plotting: very strong. The figure set remains intact, the polished notebook carries `8` decodable PNG figures and `2` decodable GIFs, and the publication-facing tables now render as deliberate report tables rather than raw dataframe dumps.
- Experimental write-up: very strong. The title card, abstract, conclusion and reproducibility appendix now tell one consistent quantitative story built around the retained-trace count, solvent sequence, rate span and exact remaining blocker.
