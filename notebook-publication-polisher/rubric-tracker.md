...
- Main uncertainty: the saved notebook now presents itself professionally and more rigorously, and the corrected notebook source finally matches that state, but the attached local review package still lacks four solvent folders, so end-to-end rerunnability from the supplied raw data is not yet demonstrated.
...
- The attached notebook source, saved rendered outputs and polished output artifact now agree, rather than leaving the durable notes ahead of the actual deliverable.
- The polished notebook now contains explicit archived-output provenance notes, a stronger primary-paper citation and a closed-by-default second GIF panel in both source and saved output.
