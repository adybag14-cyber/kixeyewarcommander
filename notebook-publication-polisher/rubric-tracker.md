...
- Data analysis + plotting: very strong in the rebuilt polished notebook. The figure set remains intact, all eight embedded PNG figures and both embedded GIF assets decode cleanly, no expandable media panel opens by default, the visible tables are captioned overflow-safe report tables without inherited index clutter, and the source now includes GIF checks alongside the existing PNG checks for future reruns.
...
- A final exported-HTML check would still be worth doing once an environment with `nbconvert` or a comparable Jupyter export stack is available.
