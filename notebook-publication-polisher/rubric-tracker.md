...
- Data analysis + plotting: very strong. The figure set remains intact, both GIF extras render cleanly, the workflow animation is now collapsed by default, the saved tables now read as captioned publication tables rather than notebook-default dataframe dumps, and all ten embedded media assets decode cleanly with no broken payloads.
