...
## 2026-05-27 live-source resync, evidence correction and refreshed media audit

- Re-opened the live attached notebook package and confirmed that the visible source had drifted back to the weaker 2017 citation, incomplete provenance wording and a chemistry-text mismatch where acetone was still described as the fastest solvent.
- Patched the actual notebook source so the title-card scope, configuration section, analysis-environment note and reproducibility appendix now state plainly that the displayed tables, figures and GIFs are archived outputs from a complete five-solvent execution, while the currently attached review package only exposes `testing-main/Data/Acetone` locally.
- Replaced reference 5 with the more defensible primary *Chemical Physics Letters* paper by Kobayashi, Yokoyama and Kamei (`10.1016/0009-2614(87)80394-9`), which explicitly addresses solvent effects in push-pull-substituted cis-azobenzenes.
- Corrected the saved results discussion, post-lab answers and conclusion so the narrative now matches the executed ordering `Acetonitrile > Acetone > THF > Cyclohexane > Toluene` and no longer treats acetone as the fastest solvent.
- Removed the default-open state from the remaining inline workflow GIF panel so both expandable animations now open in a cleaner closed state.
...
- Re-ran an embedded-media decode audit and confirmed again that all 10 saved visual assets open successfully after the edits: 8 PNG figures and 2 GIFs.
