...
- Confirmed that the attachment had slipped back to the weaker saved state: raw dataframe-style outputs were visible again, reduced-bundle provenance notes were missing from several sections, the weaker 2017 solvent-effects citation had returned, and the laboratory workflow GIF panel still opened expanded by default.
...
- Replaced all nine visible saved dataframe outputs with captioned, overflow-safe publication tables and removed the remaining default dataframe-class styling from those rendered tables.
- Updated the notebook source so future reruns emit publication-style package-audit, summary, QC, bootstrap ranking, benchmark, validation, sensitivity, and consistency-check tables with captions intact.
- Added stronger reduced-bundle provenance notes to the title card, theory section, configuration section, results framing, reproducibility appendix, and automated-check appendix.
...
- Closed the laboratory workflow animation panel by default in both notebook source and saved output, and extended the consistency-check source so reruns verify both GIF assets alongside the PNG figure set.
- Revalidated the rebuilt notebook by confirming that the stale citation is gone, no raw dataframe-style HTML tables remain, no media panel opens by default, all code cells compile, and all eight embedded PNG figures plus both embedded GIF assets decode cleanly.

## 2026-05-18 attached-package publication repair refresh and verification pass

- Reopened the attached notebook package, rubric guidance, and saved progress notes before making any publication-readiness claim.
- Confirmed that the attachment itself still showed the same high-impact defects in saved state: raw dataframe-style tables, the weaker 2017 solvent-effects citation, missing reduced-bundle provenance notes in the title/methods/appendices, and a laboratory workflow GIF panel that opened expanded by default.
- Reconfirmed the local package limitation: only `Data/Acetone/` is attached for rerun testing, so a true five-solvent rerun is still blocked in this workspace.
- Built a repeatable repair script at `/workspace/patch_publication_notebook.py` and regenerated `/workspace/output/P201_201698955_publication_ready_polished.ipynb`.
- Replaced all nine visible saved dataframe outputs with captioned, overflow-safe publication tables and stripped the remaining default dataframe class styling from the rendered HTML.
- Updated the notebook source so future reruns emit the same publication-style tables for the package audit, summary, QC, bootstrap ranking, benchmark, validation, sensitivity, and final consistency-check sections.
- Added stronger reduced-bundle provenance notes to the title card, introduction, configuration section, results framing, reproducibility appendix, and automated-check appendix.
- Replaced the weaker 2017 solvent-effects citation with the stronger 1987 *Chemical Physics Letters* paper by Kobayashi, Yokoyama and Kamei.
- Closed the workflow animation panel by default in both notebook source and saved output, and extended the consistency-check source so reruns verify both GIF assets alongside the PNG figure set.
- Revalidated the rebuilt notebook by confirming that the stale citation is gone, no raw dataframe-style HTML tables remain, no media panel opens by default, all code cells compile, and all eight embedded PNG figures plus both embedded GIF assets decode cleanly.
