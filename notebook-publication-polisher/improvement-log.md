...
## 2026-05-13 publication polish pass on attached source notebook

- Re-read the attached notebook package directly and confirmed that the attached source notebook still contained publication-level inconsistencies even though earlier saved notes described those issues as already fixed.
- Built a fresh polished deliverable at `/workspace/output/P201_201698955_publication_ready_polished.ipynb` from the attached source notebook instead of relying on stale status descriptions.
...
- Removed the default-open state from the second inline laboratory-workflow GIF in both the stored HTML output and the code that regenerates it, and confirmed that both inline animation panels are now closed by default in the polished copy.
