...
- Reopened the actual attached notebook, rather than trusting the saved notes, and confirmed that the real notebook still contained several unresolved issues: the weaker 2017 citation, an optimistic reproducibility statement, plain wide HTML tables and text that still implied acetone was the fastest solvent.
- Rebuilt `/workspace/repair_publication_notebook.py` in the current workspace so the publication repairs are reproducible from the attached notebook itself.
...
- Corrected a second narrative mismatch in the literature-comparison paragraph so the notebook now treats the JCE comparison as qualitative support rather than overstating exact agreement.
