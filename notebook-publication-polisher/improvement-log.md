...
- Confirmed that the durable notes were ahead of the actual notebook content: the stored notebook still had a generic title-card result, a generic abstract, an expanded workflow GIF panel on first load, and remaining prose that still treated acetone as the fastest retained solvent in places.
...
- Corrected the saved results-discussion wording and the generating narrative source so the mechanistic interpretation no longer contradicts the retained Acetonitrile > Acetone ordering or overstate one bulk polarity descriptor as the full explanation.
- Corrected the saved post-lab answers and the generating source so the timescale example uses acetonitrile, the solvent-order explanation is internally consistent, and acetone is described as a close second rather than the fastest retained solvent.
...
- Confirmed that the notebook source and stored rendered outputs no longer contain the stale phrases that previously described acetone as the fastest retained solvent or opened the workflow animation by default.
