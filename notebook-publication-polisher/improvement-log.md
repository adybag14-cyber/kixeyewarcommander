...
- Rewrote the opener, abstract, conclusion, reproducibility appendix, and consistency-check framing so the notebook now leads with `111 of 225` retained traces, the accepted solvent order `Acetonitrile > Acetone > THF > Cyclohexane > Toluene`, and the `46.5-fold` rate span.
