...
## 2026-05-19 publication repair and audit pass
...
- Strengthened provenance wording in the configuration section and reproducibility appendix so the reduced review bundle is not misrepresented as a complete rerunnable submission package.
...
- Extended the consistency-check wording and future rerun logic so inline GIF media are audited alongside exported PNG figures.
...
- The current container also does not provide a notebook HTML export tool, so a browser-style render check still needs a fuller environment.
