...
- Confirmed that the attached package still contained publication-quality gaps: a vague opening summary, contradictory solvent-order interpretation in both source and rendered outputs, an auto-open laboratory GIF panel, and stored HTML tables that still lacked explicit notebook-level scroll protection.
...
- Corrected the source-side results-discussion template so it no longer claims that acetone is the fastest solvent or explains the ranking with the wrong polarity statement.
...
- Added notebook-level HTML table scroll protection through the front-matter CSS so wide rendered tables gain explicit overflow handling in notebook and exported HTML views.
- Re-verified the polished copy after writing it and confirmed that all 8 embedded PNG figures and both inline GIF payloads decode successfully, both GIF panels now open closed by default, and the saved notebook still contains the full embedded figure and animation set.
