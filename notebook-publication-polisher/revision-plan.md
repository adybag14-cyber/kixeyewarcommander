# Revision Plan

## Completed in this run

- Re-reviewed the attached notebook, rubric guidance and durable notes to find the highest-impact remaining publication problems.
- Confirmed that the durable notes were ahead of the actual notebook artifact, then rebuilt the polished notebook copy locally in `/workspace/output/`.
- Added an explicit execution-provenance note, corrected the solvent-order wording in the rendered narrative and post-lab answers, strengthened the references and updated the reproducibility appendices.
- Wrapped every saved HTML table in a captioned, overflow-safe figure container to reduce clipping and horizontal spill risk.
- Closed the second inline GIF panel by default and generated a fresh contact-sheet audit from the polished notebook itself.
- Updated the durable progress files so the next run starts from the corrected notebook artifact rather than the older assumptions.

## Highest-value next steps

- Restore the missing solvent directories so the notebook can be rerun locally from raw data.
- Re-execute the notebook end to end and replace the archived saved outputs with a fresh full-data run.
- Repeat the visual audit on the regenerated figures, tables and GIFs before final publication sign-off.
