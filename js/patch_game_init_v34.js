 ...
 * - NEW: Fix TileProvider (Ap) crash.
 * - NEW: Force Worldmap (Y) initialization.
 */

(function () {
    console.log("[PATCH] V34 Script Loaded");
    ...
    function patchTileProvider() {
        ...
        for (var key in window._hx_classes) {
            ...
            if (cls && cls.prototype && cls.prototype.getTiles && cls.prototype.onTileDataLoaded) {
                console.log("[PATCH V34] Found TileProvider Class: " + key);
                ...
                if (!originalGetTiles.patched) {
                    cls.prototype.getTiles = function (a) {
                        if (!this._tiles || !this._tiles.h) {
                            // console.warn("[PATCH V34] Intercepted null _tiles in getTiles. Returning dummy.");
                            return { tiles: [], backgrounds: [], backgroundColors: [] }; // Minimal dummy
                        }
        ...
        if (window._hx_classes && attempts.createView < MAX_ATTEMPTS) {
            ...
            if (Worldmap) {
                ...
                if (!Worldmap._hexMap && attempts.worldMapSetup < 1) {
                    console.log("[PATCH V34] Calling Worldmap.Setup()...");
                ...
                if (Worldmap._hexMap && Worldmap._controller && !Worldmap._mapView) {
                    console.log("[PATCH V34] Calling Worldmap.CreateMapView()...");
                    try {
                        // Mock p._layerMap if needed?
                        // p is likely WorldmapConfig or similar global.
                        // But CreateMapView uses it. If it fails, we catch it.
                ...
                } else if (Worldmap._mapView && attempts.createView === 0) {
                    // If view exists but not shown?
                    console.log("[PATCH V34] MapView already exists.");
                    attempts.createView++;
                }
            }
        }
