 ...
 * PATCH GAME INIT V41
 * - V40 Logic
 * - NEW: Visual Debugging (Red Box on Stage, Blue Box on MapView)
 */

(function () {
    console.log("[PATCH] V41 Script Loaded");
    ...
    function patchPopups() {
        ...
        for (var i = 0; i < targets.length; i++) {
            ...
            if (cls && cls.Show && !cls.Show.patched) {
                console.log("[PATCH V41] Patching Popup Class: " + targets[i]);
        ...
        if (window._hx_classes && attempts.createView < MAX_ATTEMPTS) {
            ...
            if (Worldmap) {
                ...
                if (!Worldmap._hexMap && attempts.worldMapSetup < 1) {
                    console.log("[PATCH V41] Calling Worldmap.Setup()...");
                ...
                if (Worldmap._hexMap && Worldmap._controller && !Worldmap._mapView) {
                    console.log("[PATCH V41] Calling Worldmap.CreateMapView()...");
                ...
                // FORCE ADD TO STAGE + DEBUG
                if (Worldmap._mapView && stage) {
                    var needsAdd = (Worldmap._mapView.parent !== stage);
                    if (needsAdd) {
                        console.log("[PATCH V41] Forcing Worldmap._mapView to Stage!");
                        try { stage.addChild(Worldmap._mapView); } catch (e) { console.error(">> Force Add Failed", e); }
                    }

                    // DEBUG DRAW (Red on Stage, Blue on MapView)
                    // Haxe OpenFL Graphics: element.get_graphics().beginFill(...)
                    var sG = stage.get_graphics ? stage.get_graphics() : stage.graphics;
                    if (sG) {
                        sG.clear();
                        sG.beginFill(0xFF0000, 1);
                        sG.drawRect(50, 50, 100, 100); // Red Box 50,50
                        sG.endFill();
                    }

                    var mG = Worldmap._mapView.get_graphics ? Worldmap._mapView.get_graphics() : Worldmap._mapView.graphics;
                    if (mG) {
                        mG.clear();
                        mG.beginFill(0x0000FF, 1);
                        mG.drawRect(200, 200, 100, 100); // Blue Box 200,200 (Relative to map)
                        mG.endFill();
                    }
                }
            }
