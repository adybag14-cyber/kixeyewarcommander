...
(function () {
    console.log("[PATCH] V46 Script Loaded");
    ...
    // =========================================================================
    // 6. OPENFL LOADER MOCK
    // =========================================================================
    var loaderPatched = false;
    function patchLoader() {
        if (loaderPatched) return;
        // Try to find openfl.display.Loader
        var Loader = window.openfl && window.openfl.display && window.openfl.display.Loader;
        // Also check _hx_classes
        if (!Loader && window._hx_classes && window._hx_classes["openfl.display.Loader"]) Loader = window._hx_classes["openfl.display.Loader"];

        if (Loader && Loader.prototype && Loader.prototype.load && !Loader.prototype.load.patched) {
            console.log("[PATCH V46] Patching openfl.display.Loader");
            ...
            Loader.prototype.load = function (request) {
                ...
                if (url.indexOf(".png") !== -1 || url.indexOf(".jpg") !== -1 || url.indexOf("platoonsprites") !== -1) {
                    console.log("[PATCH V46] Mocking Asset: " + url);

                    var self = this;
                    // Create dummy content
                    try {
                        var Bitmap = window.openfl.display.Bitmap;
                        var BitmapData = window.openfl.display.BitmapData;
                        var dummyBmd = new BitmapData(32, 32, false, 0xFFFF0000); // 32x32 Red
                        var dummyBitmap = new Bitmap(dummyBmd);

                        // Set content on LoaderInfo
                        if (self.contentLoaderInfo) {
                            // Hack to set content (might be read-only in Haxe)
                            // But usually JS allows override if we are aggressive
                            ...
                            // Dispatch Complete
                            setTimeout(function () {
                                var Event = window.openfl.events.Event;
                                if (Event && self.contentLoaderInfo.dispatchEvent) {
                                    self.contentLoaderInfo.dispatchEvent(new Event("complete")); // Use string "complete" or Event.COMPLETE
                                }
                            }, 50);
                ...
                if (Worldmap) {
                    ...
                    if (Worldmap._mapView && stage) {
                        if (Worldmap._mapView.parent !== stage) {
                            // console.log("[PATCH V46] Forcing Worldmap._mapView to Stage!");
