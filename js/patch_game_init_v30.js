 ...
 * PATCH GAME INIT V30
 * - V29 logic
 * - NEW: Use GAME._instance.stage as root.
 * - NEW: Debug onDataFinishedLoading.
 * - NEW: Fallback to initLocal.
 */

(function () {
    console.log("[PATCH] V30 Script Loaded");
    ...
    setInterval(function () {
        ...
        var stage = null;
        // Try multiple stage sources
        if (window.GAME && window.GAME._instance && window.GAME._instance.stage) stage = window.GAME._instance.stage;
        else if (window._hx_classes && window._hx_classes.GAME && window._hx_classes.GAME._instance && window._hx_classes.GAME._instance.stage) stage = window._hx_classes.GAME._instance.stage;
        ...
        if (!forceAttempted) {
            ...
            if (inst) {
                console.log("[PATCH V30] Checking methods on inst:", inst);
                if (inst.onDataFinishedLoading) {
                    console.warn("[PATCH V30] Calling onDataFinishedLoading...");
                    try { inst.onDataFinishedLoading(); forceAttempted = true; } catch (e) { console.error("Call failed", e); }
                } else if (inst.initLocal) {
                    console.warn("[PATCH V30] onDataFinishedLoading missing. Calling initLocal()...");
                    try { inst.initLocal(); forceAttempted = true; } catch (e) { console.error("Call failed", e); }
                } else {
                    console.log("[PATCH V30] No useful init methods found yet.");
                }
            }
        }
        ...
        function getClassName(obj) {
            try {
                if (obj.__class__ && obj.__class__.__name__) return obj.__class__.__name__;
            } catch (e) { }
            try {
        ...
        function recursiveKill(obj, depth) {
            ...
            else if (obj.__children && obj.__children.length) count = obj.__children.length; // Direct access
