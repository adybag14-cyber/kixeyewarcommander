/**
 * PATCH GAME INIT V39
 * - V38 Logic (Ap, MAP, Managers, Reconnect Block)
 * - NEW: Shotgun Popup Suppression (PLEASEWAIT, POPUPS, PopupSystem, etc.)
 */

(function () {
    console.log("[PATCH] V39 Script Loaded");
        ...
        if (URLLoaderClass && URLLoaderClass.prototype && URLLoaderClass.prototype.load && !URLLoaderClass.prototype.load.patched) {
            ...
            URLLoaderClass.prototype.load = function (urlRequest) {
                ...
                if (url.indexOf("wc/getflags") !== -1 || url.indexOf("undefinedwc") !== -1 || url.indexOf("loadidata") !== -1 || url.indexOf("assets/json/") !== -1) {
                    // Mock data for JSON assets too effectively to prevent 404s if possible? No, hard to mock specific JSON content blindly.
    ...
    function patchPopups() {
        if (!window._hx_classes) return;
        if (popupsPatched) return; // Run periodically just in case classes lazy load? No, classes should be there.
    ...
    setInterval(function () {
        ...
        // --- POPUP REMOVAL (Clean existing ones) ---
        ...
        if (window._hx_classes && attempts.createView < MAX_ATTEMPTS) {
            ...
            if (Worldmap) {
                if (!Worldmap.reconnect.patched) {
                    Worldmap.reconnect = function () { };
                    Worldmap.reconnect.patched = true;
                }
                if (!Worldmap._hexMap && attempts.worldMapSetup < 1) {
                    console.log("[PATCH V39] Calling Worldmap.Setup()...");
                ...
                if (Worldmap._hexMap && Worldmap._controller && !Worldmap._mapView) {
                    console.log("[PATCH V39] Calling Worldmap.CreateMapView()...");
