/**
 * PATCH GAME INIT V38
 * - V37 Logic (Ap, MAP, Managers)
 * - NEW: Block Worldmap.reconnect (Stop Popup Loop)
 */

(function () {
    console.log("[PATCH] V38 Script Loaded");
            ...
            function safePatch(methodName) {
                if (MapClass[methodName] && !MapClass[methodName].patched) {
                    ...
                    MapClass[methodName] = function () {
                        try { return orig.apply(this, arguments); } catch (e) { /* suppress */ }
                    };
    ...
    // =========================================================================
    // 3. MANAGER PATCH (Fix get_signalChange crash)
    // =========================================================================
        ...
        if (window._hx_classes && attempts.createView < MAX_ATTEMPTS) {
            ...
            if (Worldmap) {
                ...
                if (!Worldmap.reconnect.patched) {
                    console.log("[PATCH V38] Patching Worldmap.reconnect...");
                    Worldmap.reconnect = function () {
                        // console.log("Blocked Worldmap.reconnect"); 
                    };
                ...
                if (!Worldmap._hexMap && attempts.worldMapSetup < 1) {
                    console.log("[PATCH V38] Calling Worldmap.Setup()...");
                ...
                if (Worldmap._hexMap && Worldmap._controller && !Worldmap._mapView) {
                    console.log("[PATCH V38] Calling Worldmap.CreateMapView()...");
                    try {
