/**
 * PATCH GAME INIT V36
 * - V35 Logic (Ap, MAP.Clear)
 * - NEW: Aggressive patching of MAP.Reset, MAP.ResetEffects
 * - GOAL: Allow Worldmap.CreateMapView to finish.
 */

(function () {
    console.log("[PATCH] V36 Script Loaded");
    ...
    function patchTileProvider() {
        ...
        for (var key in window._hx_classes) {
            ...
            if (cls && cls.prototype && cls.prototype.getTiles && cls.prototype.onTileDataLoaded) {
                console.log("[PATCH V36] Found TileProvider Class: " + key);
    ...
    // =========================================================================
    // 3. MAP CLASS PATCH (Fix L.Clear/Reset crash)
    // =========================================================================
    ...
    function patchMapClass() {
        ...
        if (MapClass) {
            console.log("[PATCH V36] Found com.cc.core.MAP. Patching Clear/Reset...");

            // Helper to patch static method safely
        ...
        if (window._hx_classes && attempts.createView < MAX_ATTEMPTS) {
            ...
            if (Worldmap) {
                ...
                if (!Worldmap._hexMap && attempts.worldMapSetup < 1) {
                    console.log("[PATCH V36] Calling Worldmap.Setup()...");
                ...
                if (Worldmap._hexMap && Worldmap._controller && !Worldmap._mapView) {
                    console.log("[PATCH V36] Calling Worldmap.CreateMapView()...");
                    try {
